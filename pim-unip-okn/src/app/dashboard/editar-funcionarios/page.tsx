...
export default function CadastrarFuncionarios() {
  ...
  useEffect(() => {
    const employeeId = (window as CustomWindow).selectedEmployeeId;

    if (employeeId) {
      const fetchEmployeeData = async () => {
        try {
          const response = await axios.get(
            `http://localhost:8000/api/v1/funcionarios/${employeeId}`
          );
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/v1/funcionarios/${formData.id_funcionario}`,
        {
